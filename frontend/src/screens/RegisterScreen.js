            ...
            <Row>
                ...
                <Col md={5} lg={5} sm={12} className="d-flex align-items-center justify-content-center text-center login-content">
                    <form onSubmit={submitHandler} className="login-form">
                        ...
                        <div className="div" style={{minWidth:'240px'}}>
                                <input required type="email" placeholder="Enter Email" value={email} className="input" onChange={(e)=>setEmail(e.target.value)}/>
                        </div>
                        </div>
                        ...
                        <Button disabled={loading ?true:false} className="login-btn" value="Login" type="submit">Sign Up {loading ? <Spinner animation="border" variant="danger"/> : "" }</Button>
                        <Link className="logindiv-links" to={redirect ? `/login?redirect=${redirect}` : '/login'}>Already an user?</Link>
                    </form>
                </Col>
            </Row></Container>
        </Container></Container>
